...
func (client *BiliClient) GetDanmaku(cid int) ([]byte, error) {
	response, err := client.SimpleGET("https://comment.bilibili.com/"+strconv.Itoa(cid)+".xml", nil)
	if err != nil {
		return nil, err
	}
	...
	return util.DecodeCompressedData(data, response.Header.Get("Content-Encoding"))
}
